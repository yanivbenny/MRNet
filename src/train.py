...
parser.add_argument('--regime', type=str, default='neutral')
...
parser.add_argument('--no-recovery_strict', dest='recovery_strict', action='store_false')
parser.add_argument('--test', action='store_true')
parser.add_argument('--multihead', action='store_true')
parser.add_argument('--multihead_mode', type=str, default=None, choices=['prob', 'eprob'])
...
def main(args):
    ...
    if not args.test:
        ...
        for split in ['val', 'test']:
            loss, acc, acc_regime = trainer.evaluate(split)
            print(f'{split} loss: {loss} - accuracy: {acc}')
            if acc_regime is not None:
                print(f'{split} In Regime:')
